...
#docker exec -u odoo -it proj-17 odoo --test-tags igtf_run -i l10n_ve_igtf --without-demo=True --stop-after-init -d test1
...
class TestIgtfSalesBook(IGTFTestCommonSaleBook):
    ...
    def test_sale_book_fields_includes_igtf_columns(self):
        """Ensure IGTF columns are added to sale book fields."""

        self.test01_payment_from_invoice_with_igtf_journal()

        wizard = self.get_sales_book_wizard()

        sale_book_fields = wizard.sale_book_fields()

        # --- 1️⃣ Validar que sea lista ---
        self.assertIsInstance(sale_book_fields, list, "sale_book_fields debe retornar una lista")

        # Convertimos a dict por nombre de field para buscar fácil
        fields_by_fieldname = {f["field"]: f for f in sale_book_fields if "field" in f}
    ...
    def test_sale_book_line_fields_with_igtf(self):
    ...
    def test_get_sale_book_field_groups_igtf_visibility(self):
        ...
        def _get_groups():
            with Form(self.env['wizard.accounting.reports']) as wiz_form:
                ...
                wiz_form.date_to = fields.Date.today()
                wizard = wiz_form.save()
            return wizard._get_sale_book_field_groups()

        # ---------------------------
        # 🟢 Caso 1: mostrar ambos
        # ---------------------------
        # company.not_show_bi_igtf_sale_order = False
        company.not_show_igtf_sale_order = False
        ...
        # ---------------------------
        # company.not_show_bi_igtf_sale_order = True
        company.not_show_igtf_sale_order = False
        ...
        # ---------------------------
        # company.not_show_bi_igtf_sale_order = False
        company.not_show_igtf_sale_order = True
